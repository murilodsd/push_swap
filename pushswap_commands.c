...
/*   Updated: 2024/07/01 14:53:39 by mde-souz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pushswap.h"

t_stack_ref	*ft_stacknewref(char name)
{
...
int	adjust_stack_a(t_stack_ref *stack_ref, int nbr_node)
{
	int	i;
	t_stack *top;
	t_stack *bottom;
...
t_stack_ref	*ft_initstack(int argc, char *argv[], char name)
{
	t_stack_ref	*stack_ref;
	t_stack *node;
	int	i;
	
	stack_ref = ft_stacknewref(name);
	i = 1;
	while (i < argc)
	{
...
void	push_all_sorted(t_stack_ref *stack_ref_from, t_stack_ref *stack_ref_to)
{
	...
	while (ft_stacksize(stack_ref_from) > 3 || (stack_ref_to->name == 'a' && ft_stacksize(stack_ref_from) > 0))
	{
